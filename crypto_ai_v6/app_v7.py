...
from engine_v7 import V7Engine
...
app = FastAPI(title="Crypto AI V7 — Resilient Edge Paper Portfolio Manager", lifespan=lifespan)
...
def root():
    return {
        "name": "Crypto AI V7",
        "mode": "PAPER_ONLY",
        "engine": "V7_RESILIENT_EDGE",
        "mission": "risk-adjusted capital growth; cash beats weak edge",
        "brains": ["TRADER", "HUNTER", "SWING", "REVERSAL"],
        "dashboard_url": "/dashboard", "status_url": "/status",
        "report_url": "/report", "health_url": "/health",
...
@app.get("/health")
def health():
    return {"version": 7, "mode": "PAPER_ONLY", **engine.health_state}
...
def export_trades_csv():
    ...
    fields = ["symbol","brain","entry_price","exit_price","qty","pnl_net","return_pct",
              "mfe_pct","mae_pct","scale_count","entry_time","exit_time","reason"]
    buf = io.StringIO()
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=crypto_ai_v7_closed_trades.csv"},
    )
